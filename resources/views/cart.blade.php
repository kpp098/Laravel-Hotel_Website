                ...
                @if ($total_price != 0)
                    @foreach ($extra_charge as $chrage)
                        <tr>
                            ...
                            <td style="text-align:center">Rs:{{ $chrage->price }}</td>
            ...
            <tfoot>
                <form method="post" action="{{ route('coupon/apply') }}">
                    ...
                    @if ($total_price == 0)
                        ...
                        <td> <input type="text" name="code" class="form-control" id="exampleFormControlInput1"
                                placeholder=""></td>
                ...
                <tr>
                    ...
                    <td colspan="5" class="text-right">
                        <h5><strong>Total Rs:{{ $without_discount_price }}</strong></h5>
                    </td>
                </tr>
                <tr>

                    <td colspan="5" class="text-right">
                        <h5><strong>Discount Rs:{{ $discount_price }}</strong></h5>
                    </td>
                </tr>
                <tr>

                    <td colspan="5" class="text-right">
                        <h3><strong>Total (With Discount) Rs:{{ $total_price }}</strong></h3>
                    </td>
                </tr>
                <tr>
                    <td colspan="5" class="text-right">
                        <a href="{{ url('/menu') }}" class="btn btn-warning"><i class="fa fa-angle-left"></i> Continue
                            Shopping</a>
                        <form style="display:inline" method="post" action="{{ route('cart.checkout', $total_price) }}">
                            @csrf
                            @if ($total_price == 0)
